...
/**
 * Will return the path and default fcash-node configuration. It will search for the
 * configuration file in the "~/.fcash" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.fcash');
  var defaultConfigFile = path.resolve(defaultPath, './fcash-node.json');
  ...
  if (fs.existsSync(defaultConfigFile)) {
    ...
    console.log(`The configuration file at '${defaultConfigFile}' is incompatible with this version of Fcash.`);

    var now = new Date();
    // fcash-node.YYYY-MM-DD.UnixTimestamp.json
    var backupFileName = `fcash-node.${now.getUTCFullYear()}-${now.getUTCMonth()}-${now.getUTCDate()}.${now.getTime()}.json`;
