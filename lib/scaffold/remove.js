...
/**
 * Will remove the Node.js service and from the fcash-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The fcash-node configuration directory
...
function remove(options, done) {
  ...
  var fcashConfigPath = path.resolve(configPath, 'fcash-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(fcashConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a fcash-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to fcash-node.json
        removeConfig(fcashConfigPath, service, next);
      });
    }, done
  );
}
