...
function lookInModuleManifest(req, service) {
  try {
    ...
    if (servicePackage.fcashNode) {
      serviceModule = serviceModule + '/' + servicePackage.fcashNode;
...
function loadModule(req, service) {
  ...
  //package.json file, if so, then see if there is a fcashNode field, if so
  ...
  if (!serviceCode) {
    throw new Error('Attempted to load the ' + service.name + ' service from: ' +
      ...
      process.cwd() + '/package.json" - fcashNode field. All paths failed to find valid nodeJS code.');
  }
